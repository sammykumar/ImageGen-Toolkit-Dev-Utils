...
class JobEventEmitterNodeTests(unittest.TestCase):
    ...
    def test_emit_and_passthrough_normalizes_sampler_typed_values_to_plain_string(self):
        ...
        self.assertTrue(any("Completion metadata preview={'sampler': 'dpmpp_2m'}" in entry for entry in logs.output))
        self.assertFalse(any("Sampler debug summary=" in entry for entry in logs.output))
    ...
    def test_emit_and_passthrough_recovers_sampler_label_from_ksampler_object(self):
        ...
        self.assertTrue(
            any(
                "Completion metadata preview={'sampler': 'euler_ancestral_cfg_pp'}" in entry
